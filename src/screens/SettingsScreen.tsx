...
export function SettingsScreen() {
  ...
  return (
    <div className="screen settings">
      ...
      <section className="settings__group">
        <span className="eyebrow">ABOUT SCROLLR</span>
        <Row emoji="💜" label="About SCROLLR" onClick={() => setOpenDoc('about')} />
      ...
      <p className="tiny settings__version">SCROLLR prototype · v{APP_VERSION}</p>
    </div>
  );
}
...
const DOCS: Record<Exclude<Doc, null>, { title: string; updated: string; body: string[] }> = {
  privacy: {
    ...
    body: [
      ...
      'Reactions are stored as counts for a round, not as a record of who reacted to what and when. SCROLLR does not keep a history of what you watched.',
      ...
      'SCROLLR does not use your camera or microphone, and does not read your photo library, unless you are explicitly asked and agree first. Nothing is recorded.',
  ...
  terms: {
    ...
    body: [
      ...
      'The feeds you watch are generated by SCROLLR itself. No TikTok, Reels or Shorts content is fetched, embedded, hosted or redistributed at any point.',
  ...
  guidelines: {
    ...
    updated: 'Plain description of this build — not reviewed legal copy',
    body: [
      'SCROLLR puts you in a room with strangers to watch things together. That only works if the room is safe.',
      ...
      'Do not share content you have no right to share, and do not use SCROLLR to promote scams or spam.',
      ...
      'SCROLLR is not intended for children. Age requirements are part of what a public launch has to settle — see LEGAL_READINESS.md in the repository.',
    ],
  },
  about: {
    title: 'About SCROLLR',
    ...
    body: [
      ...
      'SCROLLR is not a place to make videos, upload them or host them. It never has been, and the architecture is deliberately built so it does not have to become one: a video is a reference to something that lives somewhere else, and the session does not care where.',
      'This build plays SCROLLR’s own generated sample content. Accounts, profiles, friends and the directory are real when the database is connected; matchmaking and shared sessions are still being built, and lobbies are filled with bots that are labelled as bots.',
    ],
  },
};
