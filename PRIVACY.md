...
Verifiable, not just asserted: `npm run check:privacy` scans the built site for
any host outside a tiny allowlist and fails the build if it finds one. It runs
in CI and on the deploy path, so this claim is checked rather than trusted.

**This was not always true.** Until the August 2026 audit, `index.html` loaded
the Outfit typeface from `fonts.googleapis.com`, which sent every visitor's IP
address, User-Agent and Referer to Google on every page load, before any
consent and for no benefit to the visitor. The font is now self-hosted (one
32 kB variable file, same typeface), and the scanner exists so the gap cannot
reopen quietly. The lesson is worth keeping: the way this promise gets broken
is never malice, it is a font tag.
...
Generated by database triggers from the real event — a friend request, an
acceptance, a like — never asserted by a client. Until the August 2026 audit a
